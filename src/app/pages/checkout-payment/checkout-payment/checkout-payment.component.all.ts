...
export class CheckoutPaymentComponent implements OnInit, OnChanges, OnDestroy {
  ...
  selectedPaymentBeforeUpdate: string; // Nice update
  stripe: Stripe; // Nice update
  stripePaymentId = 'STRIPE_CHECKOUT_EMBEDDED'; // Nice update

  constructor(private route: ActivatedRoute, private checkoutFacade: CheckoutFacade) { }
  ...
  ngOnInit() {
    ...
    this.selectedPaymentBeforeUpdate = this.getBasketPayment(); // Nice update
    // Nice update
    if (!this.getBasketPayment()) {
      this.selectedPaymentBeforeUpdate = this.stripePaymentId;
      this.updatePaymentMethod.emit(this.selectedPaymentBeforeUpdate);
      this.loadStripeConfig();
    }
    // Nice update
    if (this.getBasketPayment() === this.stripePaymentId) {
      this.loadStripeConfig();
    }
    ...
    this.paymentForm
      ...
      .subscribe(id => {
        ...
        this.selectedPaymentBeforeUpdate = id; // Nice update
        this.checkoutFacade.clearStripeSession(); // Nice update
        // Nice update
        if (id === this.stripePaymentId) {
          this.loadStripeConfig();
        }
      });
    ...
    this.checkoutFacade.getStripeApiKey();
  }
  ...
  ngOnChanges(c: SimpleChanges) {
    ...
    if (JSON.stringify(c.paymentMethods.currentValue) !== JSON.stringify(c.paymentMethods.previousValue)) {
      // Nice update
  ...
  async loadStripeConfig() {
    this.checkoutFacade.getStripeApiKey$.pipe(takeUntil(this.destroy$)).subscribe(async (stripeApiKey: string) => {
      if (stripeApiKey) {
